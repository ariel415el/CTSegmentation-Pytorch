...
class StanfordBNet(nn.Module):
    """
    The first architecture desecribed in the paper "Semantic Segmentation using Adversarial Networks"
    """

    def __init__(self, image_channels, n_class):
        super(StanfordBNet, self).__init__()

        self.conv1_1 = nn.Conv2d(n_class, 64, 5, stride=1, padding=1)

        self.conv2_1 = nn.Conv2d(image_channels, 16, 5, stride=1, padding=1)
    ...
    def forward(self, segmented_input, original_input):
        ...
        b = segmented_input.shape[0]
        return out.view(b, -1)[:, 0]
